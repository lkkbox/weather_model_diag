...
def run(modelName, initTimes, members, clim_bc=False):
    #
    # ---- init
    # const
    rmm_tool = RMM_Tool()
    numPrevDays = 120
    srcRootDir = '../../../data/MJO/mermean_15NS'
    srcRootDirObs = f'{srcRootDir}/obs'
    if clim_bc:
        srcRootDirMod = f'{srcRootDir}/{modelName}'
        desRoot = f'../../../data/MJO/RMM/{modelName}'
    else:
        srcRootDirMod = f'{srcRootDir}/{modelName}-noBC'
        desRoot = f'../../../data/MJO/RMM/{modelName}-noBC'
