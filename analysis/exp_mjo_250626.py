...
def main():
    ...
    for caseDate in caseDates:
        ...
        modules = [
            ...
            #         'do_data': True,
            #         'do_plot': True,
            #         'fig_subdir': f'{strDate(caseDate, '%y%m%d')}',
            #         'score_diagram': {
            #             'xlim': [1, 45],
            ...
            #             'do_acc': False,
            #             'add_ensmean': False,
            #             'mpl_line_opts': [{}, {}, {}, {}, {'color': 'tab:purple'}]
            #         },
            #         'phase_diagram': {
...
def get_ps_vintqTend_map(caseDate):
    area = [120, 160, -8, 5]
    return [
        {
            'figs': [
                {
                    'title': rf'sfc-{levTop}hPa <$q_t$> init({strDate(caseDate)}) lead({leads}, {leade}])',
                    'name': f'{strDate(caseDate, '%y%m%d')}/map_vintqTend{levTop}_l{leads}-{leade}.png',
                    'mpl_opts': {'figsize': (8, 6), 'layout': 'constrained'},
                    'dim_means': [leads-1, leade-1],
                }
                for leads, leade in [
            ...
            'shadings': [
                {
                    ...
                    'levels': pyt.ct.mirror([0.1, 0.4, 0.8, 1.2, 1.6, 2]),
                    'colormap': pyt.colormaps.nclColormap('CBR_coldhot'),
                    'smooths': [2.5, 2.5],
                    'contour_opts': {'linestyles': '-', 'colors': 'grey', 'linewidths':0.5},
                    'amean': area,
...
def get_ps_vintq_largeMap(caseDate):
    return [
        {
            'figs': [
                {
                    'title': f'sfc-{levTop}hPa <q> init({strDate(caseDate)}) lead({leads}, {leade}])',
                    'name': f'{strDate(caseDate, '%y%m%d')}/map_vintq{levTop}_large_l{leads}-{leade}.png',
                    'mpl_opts': {'figsize': (8, 6), 'layout': 'constrained'},
                    'dim_means': [leads-1, leade-1],
                }
                for leads, leade in [
            ...
            'shadings': [
                {
                    'variable': 'q',
                    'xy_axis': [-1, -2],
                    'minMaxs': [[0, 45], [levTop, 1000], [-40, 40], [30, 240]],
                    'total_anomaly': 'anomaly',
                    # 'math': lambda z: z * 1000,
                    'math': lambda z: z / 9.8,
                    'operators': ['vertical_pressure_integration'],
                    'levels': pyt.ct.mirror([0.5, 1, 1.5, 3, 4.5, 6, 8, 10]),
                    'colormap': pyt.colormaps.nclColormap('CBR_coldhot'),
                    'smooths': [2.5, 2.5],
                    'contour_opts': {'linestyles': '-', 'colors': 'grey', 'linewidths':0.5},
                    # 'amean': area,
    ...
    return [
        {
            'figs': [
                {
                    'title': fr'sfc-{levTop}hPa q, $q_t$, init({strDate(caseDate)}), {area=}',
            ...
            'lines': [
                ...
                },
                {
                    ...
                    'minMaxs': [[0, 45], [levTop, 1000], area[2:], area[:2]],
                    'total_anomaly': 'anomaly',
                    'math': lambda z: z / 9.8 * 3,
                    'operators': ['time_tendency', 'vertical_pressure_integration'],
                    'smooths': [3, None],
                    'mpl_opts': {'color': 'black', 'linestyle': '--'},
                },
            ],
        }
        for levTop in [850, 700]
...
def get_ps_olr_hov(caseDate):
    return  [
        {
            'figs': [
                {
                    'title': f'OLR init({strDate(caseDate)}) lat(15S-15N)',
                    'name': f'{strDate(caseDate, '%y%m%d')}/lon_time_olr_{total_anomaly}.png',
                    'mpl_opts': {'figsize': (6.4, 4.8), 'layout': 'constrained'},
                },
            ],
            'world_tick_dx': 30,
...
def get_ps_prec_hov(caseDate):
    return  [
        {
            'figs': [
                {
                    'title': f'Prec init({strDate(caseDate)}) lat(15S-15N)',
                    'name': f'{strDate(caseDate, '%y%m%d')}/lon_time_prec_{total_anomaly}.png',
                    'mpl_opts': {'figsize': (6.4, 4.8), 'layout': 'constrained'},
                },
            ],
            'world_tick_dx': 30,
...
def get_ps_u_hov(caseDate):
    return [
        {
            'figs':[ 
                {
                    'title': f'u{pLev} init({strDate(caseDate)}) lat(15S-15N)',
                    'name': f'{strDate(caseDate, '%y%m%d')}/lon_time_u{pLev}_anomaly.png',
                    'mpl_opts': {'figsize': (6.4, 4.8), 'layout': 'constrained'},
                },
            ],
            'world_tick_dx': 30,
            ...
            'shadings': [
                {
                    'variable': 'u',
                    'xy_axis': [-1, -4],
                    'minMaxs': [[0, 45], [pLev]*2, [-15, 15], [60, 210]],
                    'total_anomaly': 'anomaly',
                    'levels': cLev,
                    'colormap': pyt.colormaps.nclColormap('CBR_coldhot'),
                    'smooths': [5, 5],
        ...
        for pLev, cLev in [
            (850, pyt.ct.mirror([1, 3, 5, 7, 9, 12])),
            (200, pyt.ct.mirror([2, 4, 6, 8, 12, 16])),
        ]
    ]
